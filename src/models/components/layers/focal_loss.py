...
class Vanilla_FocalLoss(nn.Module):
    ...
    def forward(self, prediction, target):
        ...
        loss = 0.0
        positive_loss = (
            torch.log(prediction) * torch.pow(target - prediction, self.gamma) * positive_index
        )

        negative_loss = (
            torch.log(1 - prediction) * torch.pow(prediction, self.gamma) * negative_index
        )
...
class FocalLoss(nn.Module):
    ...
    def forward(self, prediction, target):
        ...
        positive_loss = (
            torch.log(prediction) * torch.pow(1 - prediction, self.alpha) * positive_index
        )
