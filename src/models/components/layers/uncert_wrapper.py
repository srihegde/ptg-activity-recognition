...
class MultiTaskLossesWrapper(nn.Module):
    def __init__(self, use_uncert=True, reduce_loss=True, keys=[], uncertainty_keys=[]):
        ...
        self.keys = keys
        self.uncertainty_keys = [
            key for key, uncertainty in zip(keys, uncertainty_keys) if uncertainty
        ]
    ...
    def forward(self, loss_dict):
        ...
        for idx, key in enumerate(self.keys):
            if key in self.uncertainty_keys:
                ...
                loss_dict[key] = (
                    loss_dict[key] * torch.exp(-self.log_vars[uncertainty_idx])
                    + self.log_vars[uncertainty_idx]
                )
