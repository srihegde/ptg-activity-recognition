import torch
from engine.visualize_infer import box3d_to_corners, box_iou, box_iou_3d
from model.anno_encoder import Anno_Encoder
from model.layers.utils import Converter_key2channel, nms_hm, select_point_of_interest, select_topk
...
def make_post_processor(cfg):
    ...
    key2channel = Converter_key2channel(
...
class PostProcessor(nn.Module):
    ...
    def prepare_targets(self, targets, test):
        ...
        if test:
            return dict(calib=calibs, size=size, pad_size=pad_size)
    ...
    def forward(self, predictions, targets, features=None, test=False, refine_module=None):
        ...
        dis_ious = (
            self.evaluate_3D_detection(target_varibales, pred_regression)
        ...
        depth_errors = (
            self.evaluate_3D_depths(target_varibales, pred_regression) if self.eval_depth else None
        )
        ...
        pred_regression_pois = select_point_of_interest(batch, indexs, pred_regression).view(
            -1, pred_regression.shape[1]
        )
        ...
        pred_orientation = torch.cat(
            (
                pred_regression_pois[:, self.key2channel("ori_cls")],
        ...
        pred_box2d = self.anno_encoder.decode_box2d_fcos(
            pred_bbox_points, pred_2d_reg, pad_size, img_size
        )
        ...
        if self.depth_with_uncertainty:
            pred_direct_uncertainty = pred_regression_pois[
                :, self.key2channel("depth_uncertainty")
            ].exp()
            visualize_preds["depth_uncertainty"] = pred_regression[
                :, self.key2channel("depth_uncertainty"), ...
            ].squeeze(1)
        ...
        if self.regress_keypoints:
            ...
            pred_keypoints_depths = self.anno_encoder.decode_depth_from_keypoints_batch(
                pred_keypoint_offset, pred_dimensions, calib
            )
            visualize_preds["keypoints"] = pred_keypoint_offset

        if self.keypoint_depth_with_uncertainty:
            pred_keypoint_uncertainty = pred_regression_pois[
        ...
        # hard ensemble, soft ensemble and simple average
            ...
            if self.pred_direct_depth and self.depth_with_uncertainty:
                pred_combined_depths = torch.cat(
                    (pred_direct_depths.unsqueeze(1), pred_keypoints_depths), dim=1
                )
                pred_combined_uncertainty = torch.cat(
                    (pred_direct_uncertainty, pred_keypoint_uncertainty), dim=1
                )
            else:
            ...
            if self.output_depth == "hard":
                pred_depths = pred_combined_depths[
                    torch.arange(pred_combined_depths.shape[0]), depth_weights.argmax(dim=1)
                ]
            ...
            # the best estimator is always selected
                ...
                pred_depths, estimated_depth_error = self.get_oracle_depths(
                    pred_box2d, clses, pred_combined_depths, pred_combined_uncertainty, targets[0]
                )

        batch_idxs = pred_depths.new_zeros(pred_depths.shape[0]).long()
        pred_locations = self.anno_encoder.decode_location_flatten(
            pred_bbox_points, pred_offset_3D, pred_depths, calib, pad_size, batch_idxs
        )
        pred_rotys, pred_alphas = self.anno_encoder.decode_axes_orientation(
    ...
    def get_oracle_depths(
        self, pred_bboxes, pred_clses, pred_combined_depths, pred_combined_uncertainty, target
    ):
        ...
        for i in range(pred_bboxes.shape[0]):
            ...
            # iou 2d
            iou_2d = box_iou(
                box2d.detach().cpu().numpy(), gt_boxes[near_idx].detach().cpu().numpy()
            )
            ...
            if iou_2d < iou_thresh:
                ...
                estimator_index = torch.argmin(
                    torch.abs(pred_combined_depths[i] - gt_depths[near_idx])
                )
    ...
    def evaluate_3D_depths(self, targets, pred_regression):
        ...
        pred_keypoints_depths = self.anno_encoder.decode_depth_from_keypoints_batch(
            pred_keypoint_offset.view(-1, 10, 2), pred_dimensions, targets["calib"]
        )
        ...
        pred_uncertainty_softmax_depth = torch.sum(
    ...
    def evaluate_3D_detection(self, targets, pred_regression):
        ...
        pred_keypoint_offset = pred_regression_pois[:, self.key2channel("corner_offset")].view(
            -1, 10, 2
        )
        ...
        if self.output_depth == "direct":
            ...
            pred_depths = self.anno_encoder.decode_depth_from_keypoints(
                pred_offset_3D, pred_keypoint_offset, pred_dimensions, targets["calib"]
            )
            pred_uncertainty = pred_regression_pois[
                :, self.key2channel("corner_uncertainty")
            ].exp()
            pred_depths = pred_depths[
                torch.arange(pred_depths.shape[0]), pred_uncertainty.argmin(dim=1)
            ]

        elif self.output_depth == "combine":
            pred_direct_depths = self.anno_encoder.decode_depth(pred_depths_offset)
            pred_keypoints_depths = self.anno_encoder.decode_depth_from_keypoints(
                pred_offset_3D, pred_keypoint_offset, pred_dimensions, targets["calib"]
            )
            pred_combined_depths = torch.cat(
                (pred_direct_depths.unsqueeze(1), pred_keypoints_depths), dim=1
        ...
        batch_idxs = pred_depths.new_zeros(pred_depths.shape[0]).long()
        pred_locations_offset = self.anno_encoder.decode_location_flatten(
            target_points,
            pred_offset_3D,
            target_depths,
            targets["calib"],
        ...
        pred_locations_depth = self.anno_encoder.decode_location_flatten(
            target_points,
            target_offset_3D,
            pred_depths,
            targets["calib"],
        ...
        fully_pred_rotys, _ = self.anno_encoder.decode_axes_orientation(
            pred_orientation, pred_locations
        )

        # fully predicted
        pred_bboxes_3d = torch.cat(
            (pred_locations, pred_dimensions, fully_pred_rotys[:, None]), dim=1
        )
        # ground-truth
        target_bboxes_3d = torch.cat(
            (target_locations, target_dimensions, target_rotys[:, None]), dim=1
        )
        # disentangling
        offset_bboxes_3d = torch.cat(
            (pred_locations_offset, target_dimensions, target_rotys[:, None]), dim=1
        )
        depth_bboxes_3d = torch.cat(
            (pred_locations_depth, target_dimensions, target_rotys[:, None]), dim=1
        )
        dims_bboxes_3d = torch.cat(
            (target_locations, pred_dimensions, target_rotys[:, None]), dim=1
        )
        orien_bboxes_3d = torch.cat(
            (target_locations, target_dimensions, pred_rotys[:, None]), dim=1
        )
...
def get_iou3d(pred_bboxes, target_bboxes):
    ...
    for i in range(num_query):
        ...
        union3d = (
            ref_polygon.area * (max_h_a[0] - min_h_a[0])
            + target_polygon.area * (max_h_b[i] - min_h_b[i])
            - overlap3d
        )
