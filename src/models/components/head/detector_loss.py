...
from model.layers.utils import Converter_key2channel, select_point_of_interest
from model.utils import Laplace_Loss, Uncertainty_Reg_Loss
...
class Loss_Computation:
    def __init__(self, cfg):
        ...
        self.key2channel = Converter_key2channel(
            keys=cfg.MODEL.HEAD.REGRESSION_HEADS, channels=cfg.MODEL.HEAD.REGRESSION_CHANNELS
        )
        ...
        self.cls_loss_fnc = FocalLoss(
            cfg.MODEL.HEAD.LOSS_PENALTY_ALPHA, cfg.MODEL.HEAD.LOSS_BETA
        ...
        self.loss_weights = {}
        for key, weight in zip(cfg.MODEL.HEAD.LOSS_NAMES, cfg.MODEL.HEAD.INIT_LOSS_WEIGHT):
            self.loss_weights[key] = weight
    ...
    def prepare_predictions(self, targets_variables, predictions):
        ...
        target_regression_2D = torch.cat(
            (
                valid_targets_bbox_points - target_bboxes_2D[:, :2],
                target_bboxes_2D[:, 2:] - valid_targets_bbox_points,
            ),
            dim=1,
        )
        ...
        target_orientation_3D = targets_variables["orientations"].view(
            -1, targets_variables["orientations"].shape[-1]
        )[flatten_reg_mask_gt]
        target_locations_3D = self.anno_encoder.decode_location_flatten(
        ...
        target_corners_3D = self.anno_encoder.encode_box3d(
            target_rotys_3D, target_dimensions_3D, target_locations_3D
        )
        target_bboxes_3D = torch.cat(
            (target_locations_3D, target_dimensions_3D, target_rotys_3D[:, None]), dim=1
        )
        ...
        pred_regression_pois_3D = select_point_of_interest(
            batch, targets_bbox_points, pred_regression
        ).view(-1, channel)[flatten_reg_mask_gt]

        pred_regression_2D = F.relu(
            pred_regression_pois_3D[mask_regression_2D, self.key2channel("2d_dim")]
        )
        ...
        pred_orientation_3D = torch.cat(
            (
                pred_regression_pois_3D[:, self.key2channel("ori_cls")],
                pred_regression_pois_3D[:, self.key2channel("ori_offset")],
            ),
            dim=1,
        )

        # decode the pred residual dimensions to real dimensions
        pred_dimensions_3D = self.anno_encoder.decode_dimension(
            target_clses, pred_dimensions_offsets_3D
        )

        # preparing outputs
        targets = {
            "reg_2D": target_regression_2D,
            "offset_3D": target_offset_3D,
            "depth_3D": target_depths_3D,
            "orien_3D": target_orientation_3D,
            "dims_3D": target_dimensions_3D,
            "corners_3D": target_corners_3D,
            "width_2D": target_bboxes_width,
            "rotys_3D": target_rotys_3D,
            "cat_3D": target_bboxes_3D,
            "trunc_mask_3D": target_trunc_mask,
            "height_2D": target_bboxes_height,
        }

        preds = {
            "reg_2D": pred_regression_2D,
            "offset_3D": pred_offset_3D,
            "orien_3D": pred_orientation_3D,
            "dims_3D": pred_dimensions_3D,
        }

        reg_nums = {
            "reg_2D": mask_regression_2D.sum(),
            "reg_3D": flatten_reg_mask_gt.sum(),
        ...
        if self.pred_direct_depth:
            pred_depths_offset_3D = pred_regression_pois_3D[:, self.key2channel("depth")].squeeze(
                -1
            )
        ...
        if self.depth_with_uncertainty:
            preds["depth_uncertainty"] = pred_regression_pois_3D[
                :, self.key2channel("depth_uncertainty")
            ].squeeze(-1)

            if self.uncertainty_range is not None:
                preds["depth_uncertainty"] = torch.clamp(
                    preds["depth_uncertainty"],
        ...
        if self.compute_keypoint_corner:
            ...
            target_corner_keypoints = targets_variables["keypoints"].view(
                flatten_reg_mask_gt.shape[0], -1, 3
            )[flatten_reg_mask_gt]
            ...
            target_corner_depth_mask = targets_variables["keypoints_depth_mask"].view(-1, 3)[
                flatten_reg_mask_gt
            ]
            ...
            pred_keypoints_depths_3D = self.anno_encoder.decode_depth_from_keypoints_batch(
                pred_keypoints_3D, pred_dimensions_3D, targets_variables["calib"], batch_idxs
            )
        ...
        if self.corner_with_uncertainty:
            preds["corner_offset_uncertainty"] = pred_regression_pois_3D[
                :, self.key2channel("corner_uncertainty")
            ]

            if self.uncertainty_range is not None:
                preds["corner_offset_uncertainty"] = torch.clamp(
        ...
        if self.corner_loss_depth == "direct":
            ...
            pred_combined_depths = torch.cat(
                (pred_direct_depths_3D.unsqueeze(-1), preds["keypoints_depths"]), dim=1
            )

            if self.corner_loss_depth == "soft_combine":
                pred_uncertainty_weights = 1 / pred_combined_uncertainty
                pred_uncertainty_weights = pred_uncertainty_weights / pred_uncertainty_weights.sum(
                    dim=1, keepdim=True
                )
                pred_corner_depth_3D = torch.sum(
                    pred_combined_depths * pred_uncertainty_weights, dim=1
                )
                ...
                pred_corner_depth_3D = pred_combined_depths[
                    torch.arange(pred_combined_depths.shape[0]),
                    pred_combined_uncertainty.argmin(dim=1),
                ]

        # compute the corners
        pred_locations_3D = self.anno_encoder.decode_location_flatten(
            valid_targets_bbox_points,
            pred_offset_3D,
        ...
        pred_rotys_3D, _ = self.anno_encoder.decode_axes_orientation(
            pred_orientation_3D, pred_locations_3D
        )
        # encode corners
        pred_corners_3D = self.anno_encoder.encode_box3d(
            pred_rotys_3D, pred_dimensions_3D, pred_locations_3D
        )
        # concatenate all predictions
        pred_bboxes_3D = torch.cat(
            (pred_locations_3D, pred_dimensions_3D, pred_rotys_3D[:, None]), dim=1
        )

        preds.update(
            {"corners_3D": pred_corners_3D, "rotys_3D": pred_rotys_3D, "cat_3D": pred_bboxes_3D}
        )
    ...
    def __call__(self, predictions, targets):
        ...
        pred_targets, preds, reg_nums, weights = self.prepare_predictions(
            targets_variables, predictions
        )
        ...
        if num_reg_3D > 0:
            ...
            if self.compute_direct_depth_loss:
                depth_3D_loss = self.loss_weights["depth_loss"] * self.depth_loss(
                    preds["depth_3D"], pred_targets["depth_3D"], reduction="none"
                )
                real_depth_3D_loss = depth_3D_loss.detach().mean()

                if self.depth_with_uncertainty:
                    depth_3D_loss = (
                        depth_3D_loss * torch.exp(-preds["depth_uncertainty"])
            ...
            # offset_3D loss
            offset_3D_loss = self.reg_loss_fnc(
                preds["offset_3D"], pred_targets["offset_3D"], reduction="none"
            ).sum(dim=1)
            ...
            if self.separate_trunc_offset:
                ...
                trunc_offset_loss = (
                    self.loss_weights["trunc_offset_loss"]
                    * trunc_offset_loss.sum()
                    / torch.clamp(trunc_mask.sum(), min=1)
                )
                offset_3D_loss = (
                    self.loss_weights["offset_loss"] * offset_3D_loss[~trunc_mask].mean()
                )
            else:
            ...
            # orientation loss
            if self.multibin:
                orien_3D_loss = self.loss_weights["orien_loss"] * Real_MultiBin_loss(
                    preds["orien_3D"], pred_targets["orien_3D"], num_bin=self.orien_bin_size
                )

            # dimension loss
            dims_3D_loss = self.reg_loss_fnc(
                preds["dims_3D"], pred_targets["dims_3D"], reduction="none"
            ) * self.dim_weight.type_as(preds["dims_3D"])
            ...
            if self.compute_corner_loss:
                # N x 8 x 3
                corner_3D_loss = (
                    self.loss_weights["corner_loss"]
                    * self.reg_loss_fnc(
                        preds["corners_3D"], pred_targets["corners_3D"], reduction="none"
                    )
                    .sum(dim=2)
                    .mean()
                )
            ...
            if self.compute_keypoint_corner:
                # N x K x 3
                keypoint_loss = (
                    self.loss_weights["keypoint_loss"]
                    * self.keypoint_loss_fnc(
                        preds["keypoints"], pred_targets["keypoints"], reduction="none"
                    ).sum(dim=2)
                    * pred_targets["keypoints_mask"]
                )

                keypoint_loss = keypoint_loss.sum() / torch.clamp(
                    pred_targets["keypoints_mask"].sum(), min=1
                )

                if self.compute_keypoint_depth_loss:
                    pred_keypoints_depth, keypoints_depth_mask = (
                        preds["keypoints_depths"],
                        pred_targets["keypoints_depth_mask"].bool(),
                    )
                    ...
                    invalid_pred_keypoints_depth = pred_keypoints_depth[
                        ~keypoints_depth_mask
                    ].detach()

                    # valid and non-valid
                    valid_keypoint_depth_loss = self.loss_weights[
                        "keypoint_depth_loss"
                    ] * self.reg_loss_fnc(
                    ...
                    invalid_keypoint_depth_loss = self.loss_weights[
                        "keypoint_depth_loss"
                    ] * self.reg_loss_fnc(
                    ...
                    if self.corner_with_uncertainty:
                        ...
                        invalid_uncertainty = pred_keypoint_depth_uncertainty[
                            ~keypoints_depth_mask
                        ]

                        valid_keypoint_depth_loss = (
                            valid_keypoint_depth_loss * torch.exp(-valid_uncertainty)
                            + self.loss_weights["keypoint_depth_loss"] * valid_uncertainty
                        )

                        invalid_keypoint_depth_loss = invalid_keypoint_depth_loss * torch.exp(
                            -invalid_uncertainty
                        )
                    ...
                    if self.modify_invalid_keypoint_depths:
                        keypoint_depth_loss = (
                            valid_keypoint_depth_loss + invalid_keypoint_depth_loss
                        )
                    else:
                ...
                keypoint_MAE = (
                    preds["keypoints_depths"] - pred_targets["depth_3D"].unsqueeze(-1)
                ).abs() / pred_targets["depth_3D"].unsqueeze(-1)
                ...
                if self.corner_with_uncertainty:
                    if self.pred_direct_depth and self.depth_with_uncertainty:
                        combined_depth = torch.cat(
                            (preds["depth_3D"].unsqueeze(1), preds["keypoints_depths"]), dim=1
                        )
                        combined_uncertainty = torch.cat(
                            (
                                preds["depth_uncertainty"].unsqueeze(1),
                                preds["corner_offset_uncertainty"],
                            ),
                            dim=1,
                        ).exp()
                    ...
                    # the hard ensemble
                    hard_MAE = combined_MAE[
                        torch.arange(combined_MAE.shape[0]), combined_uncertainty.argmin(dim=1)
                    ]
                    # the soft ensemble
                    ...
                    soft_MAE = (soft_depths - pred_targets["depth_3D"]).abs() / pred_targets[
                        "depth_3D"
                    ]
                    # the average ensemble
                    mean_depths = combined_depth.mean(dim=1)
                    mean_MAE = (mean_depths - pred_targets["depth_3D"]).abs() / pred_targets[
                        "depth_3D"
                    ]

                    # average
                    lower_MAE, hard_MAE, soft_MAE, mean_MAE = (
                        lower_MAE.mean(),
                        hard_MAE.mean(),
                        soft_MAE.mean(),
                        mean_MAE.mean(),
                    ...
                    if self.compute_weighted_depth_loss:
                        soft_depth_loss = self.loss_weights[
                            "weighted_avg_depth_loss"
                        ] * self.reg_loss_fnc(
                            soft_depths, pred_targets["depth_3D"], reduction="mean"
                        )
...
def Real_MultiBin_loss(vector_ori, gt_ori, num_bin=4):
    ...
    for i in range(num_bin):
        ...
        cls_ce_loss = F.cross_entropy(
            vector_ori[:, (i * 2) : (i * 2 + 2)], gt_ori[:, i].long(), reduction="none"
        )
        # regression loss
        ...
        if valid_mask_i.sum() > 0:
            ...
            reg_loss = F.l1_loss(
                pred_offset[:, 0], torch.sin(gt_ori[valid_mask_i, num_bin + i]), reduction="none"
            ) + F.l1_loss(
                pred_offset[:, 1], torch.cos(gt_ori[valid_mask_i, num_bin + i]), reduction="none"
            )
