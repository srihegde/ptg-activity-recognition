...
class Depth_Refine_Loss(nn.Module):
    def __init__(self, bin_num, bin_size, device):
        ...
        self.bin_offset = (
            torch.arange(-bin_num / 2, bin_num / 2).ceil().to(device).view(1, -1) * bin_size
        )
    ...
    def forward(self, pred_depths, pred_refine, target_depths):
        ...
        refine_cls_loss = F.cross_entropy(
            input=pred_refine_cls, target=target_bin_idxs, reduction="none"
        )
        # clamp depth error
        depth_offset = (
            torch.gather(depth_error, 1, target_bin_idxs[:, None])
            .clamp(min=-self.bin_size / 2, max=self.bin_size / 2)
            .squeeze()
        )
