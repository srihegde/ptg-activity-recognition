...
from model.layers.utils import select_point_of_interest, sigmoid_hm
from model.make_layers import _fill_fc_weights, group_norm
    ...
    def __init__(self, cfg, in_channels):
        ...
        if self.use_inplace_abn:
            self.class_head = nn.Sequential(
                ...
                InPlaceABN(
                    self.head_conv, momentum=self.bn_momentum, activation=self.abn_activision
                ),
        ...
        for idx, regress_head_key in enumerate(self.regression_head_cfg):
            ...
            for key_index, key in enumerate(regress_head_key):
                ...
                if key == "3d_offset":
                    self.offset_index = [idx, key_index]
        ...
        if self.enable_edge_fusion:
            trunc_norm_func = (
                nn.BatchNorm1d if cfg.MODEL.HEAD.EDGE_FUSION_NORM == "BN" else nn.Identity
            )
            trunc_activision_func = (
                nn.ReLU(inplace=True) if self.edge_fusion_relu else nn.Identity()
            )
            ...
            self.trunc_heatmap_conv = nn.Sequential(
                ...
                trunc_norm_func(self.head_conv, momentum=self.bn_momentum),
                trunc_activision_func,
                nn.Conv1d(self.head_conv, classes, kernel_size=1),
            )
    ...
    def forward(self, features, targets):
        ...
        for i, reg_feature_head in enumerate(self.reg_features):
            ...
            for j, reg_output_head in enumerate(self.reg_heads[i]):
                ...
                if (
                    self.enable_edge_fusion
                    and i == self.offset_index[0]
                    and j == self.offset_index[1]
                ):
                    edge_indices = torch.stack(
                        [t.get_field("edge_indices") for t in targets]
                    )  # B x K x 2
                    ...
                    edge_features = F.grid_sample(
                        feature_for_fusion, grid_edge_indices, align_corners=True
                    ).squeeze(-1)
                    ...
                    for k in range(b):
                        ...
                        ] += edge_cls_output[k, :, : edge_lens[k]]
                        output_reg[
                            k, :, edge_indice_k[:, 1], edge_indice_k[:, 0]
                        ] += edge_offset_output[k, :, : edge_lens[k]]
