...
def append_img_feat(img_feat_list, end_points):
    ...
    for img_feat in img_feat_list:
        # Rotate with Rtilt
        img_feat_xyz_camera = torch.cat(
            (img_feat[:, :, 0:2], torch.zeros((batch_size, num_seed, 1)).cuda()), -1
        )
        ...
        ray_angle /= torch.sqrt(torch.sum(ray_angle**2, -1) + 1e-6).unsqueeze(
            -1
        )  # normalize the angle
        img_mask = img_feat[:, :, -1].unsqueeze(
            -1
        )  # zero out the angle for those out of any 2d boxes
        ray_angle *= img_mask

        # Compute C''
        new_img_feat_xz_upright = torch.zeros((batch_size, num_seed, 2)).cuda()
        new_img_feat_xz_upright[:, :, 0] = (
            ray_angle[:, :, 0] / (ray_angle[:, :, 1] + 1e-6) * xyz[:, :, 1] - xyz[:, :, 0]
        )
        new_img_feat_xz_upright[:, :, 1] = (
            ray_angle[:, :, 2] / (ray_angle[:, :, 1] + 1e-6) * xyz[:, :, 1] - xyz[:, :, 2]
        )
        ...
        sub_feat_list = [
            fp2_features,
            new_img_feat_xz_upright.transpose(2, 1),
            ray_angle.transpose(2, 1),
        ]

        # Semantic cues
        img_feat_sem = img_feat[:, :, 4].long()
        img_feat_sem = torch.gather(
            semantic_cues, 1, img_feat_sem.unsqueeze(-1).repeat(1, 1, semantic_cues.shape[-1])
        ).transpose(2, 1)
        ...
        img_feat_tex = (
            img_feat_tex[:, :, 0]
            + img_feat_tex[:, :, 1] * end_points["full_img_width"].unsqueeze(-1)
        ) * 3
...
class ImageFeatureModule(nn.Module):
    ...
    def forward(self, end_points):
        xyz2 = torch.matmul(
            end_points["calib_Rtilt"].transpose(2, 1),
            (1 / (end_points["scale"] ** 2)).unsqueeze(-1).unsqueeze(-1)
            * end_points["fp2_xyz"].transpose(2, 1),
        )
        ...
        idx_beg = (
            u.float() + v.float() * end_points["full_img_width"].unsqueeze(-1).float()
        ) * self.vote_dims
        ...
        for i in range(self.max_imvote_per_pixel):
            ...
            seed_gt_votes_i *= xyz2[:, :, 2].unsqueeze(
                -1
            )  # NOTE: this depth may be very off since points in the 2D box may belong to the background.
            ...
            img_feat_list_i = torch.cat(
                (
                    seed_gt_votes_i,
                    u.unsqueeze(-1),
                    v.unsqueeze(-1),
                    ins_id,
                    seed_gt_votes_mask_i.unsqueeze(-1),
                ),
                -1,
            )
