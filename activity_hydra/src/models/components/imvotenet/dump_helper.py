...
def dump_results(end_points, dump_dir, config, inference_switch=False, key_prefix="pc_only_"):
    ...
    aggregated_vote_xyz = end_points[key_prefix + "aggregated_vote_xyz"].detach().cpu().numpy()
    objectness_scores = (
        end_points[key_prefix + "objectness_scores"].detach().cpu().numpy()
    )  # (B,K,2)
    pred_center = end_points[key_prefix + "center"].detach().cpu().numpy()  # (B,K,3)
    pred_heading_class = torch.argmax(
        end_points[key_prefix + "heading_scores"], -1
    )  # B,num_proposal
    pred_heading_residual = torch.gather(
        end_points[key_prefix + "heading_residuals"], 2, pred_heading_class.unsqueeze(-1)
    )  # B,num_proposal,1
    ...
    pred_size_residual = torch.gather(
        end_points[key_prefix + "size_residuals"],
        2,
    ...
    for i in range(batch_size):
        ...
        pc_util.write_ply(
            seed_xyz[i, :, :],
            os.path.join(dump_dir, key_prefix + "%06d_seed_pc.ply" % (idx_beg + i)),
        )
        pc_util.write_ply(
            end_points[key_prefix + "vote_xyz"][i, :, :],
            os.path.join(dump_dir, key_prefix + "%06d_vgen_pc.ply" % (idx_beg + i)),
        )
        pc_util.write_ply(
            aggregated_vote_xyz[i, :, :],
            os.path.join(dump_dir, key_prefix + "%06d_aggregated_vote_pc.ply" % (idx_beg + i)),
        )
        pc_util.write_ply(
            aggregated_vote_xyz[i, :, :],
            os.path.join(dump_dir, key_prefix + "%06d_aggregated_vote_pc.ply" % (idx_beg + i)),
        ...
        pc_util.write_ply(
            pred_center[i, :, 0:3],
            os.path.join(dump_dir, key_prefix + "%06d_proposal_pc.ply" % (idx_beg + i)),
        )
        if np.sum(objectness_prob > DUMP_CONF_THRESH) > 0:
            pc_util.write_ply(
                pred_center[i, objectness_prob > DUMP_CONF_THRESH, 0:3],
                os.path.join(
                    dump_dir, key_prefix + "%06d_confident_proposal_pc.ply" % (idx_beg + i)
                ),
            )
        ...
        if np.sum(objectness_prob > DUMP_CONF_THRESH) > 0:
            ...
            if len(obbs) > 0:
                ...
                pc_util.write_oriented_bbox(
                    ...
                    os.path.join(
                        dump_dir, key_prefix + "%06d_pred_confident_bbox.ply" % (idx_beg + i)
                    ),
                )
                pc_util.write_oriented_bbox(
                    obbs[
                    ...
                    os.path.join(
                        dump_dir, key_prefix + "%06d_pred_confident_nms_bbox.ply" % (idx_beg + i)
                    ),
                )
                pc_util.write_oriented_bbox(
                    obbs[pred_mask[i, :] == 1, :],
                    os.path.join(dump_dir, key_prefix + "%06d_pred_nms_bbox.ply" % (idx_beg + i)),
                )
                pc_util.write_oriented_bbox(
                    obbs, os.path.join(dump_dir, key_prefix + "%06d_pred_bbox.ply" % (idx_beg + i))
                )
    ...
    for i in range(batch_size):
        if np.sum(objectness_label[i, :]) > 0:
            pc_util.write_ply(
                ...
                os.path.join(
                    dump_dir, key_prefix + "%06d_gt_positive_proposal_pc.ply" % (idx_beg + i)
                ),
            )
        if np.sum(objectness_mask[i, :]) > 0:
            pc_util.write_ply(
                ...
                os.path.join(
                    dump_dir, key_prefix + "%06d_gt_mask_proposal_pc.ply" % (idx_beg + i)
                ),
            )
        pc_util.write_ply(
            gt_center[i, :, 0:3],
            os.path.join(dump_dir, key_prefix + "%06d_gt_centroid_pc.ply" % (idx_beg + i)),
        )
        pc_util.write_ply_color(
            pred_center[i, :, 0:3],
            objectness_label[i, :],
            os.path.join(
                dump_dir, key_prefix + "%06d_proposal_pc_objectness_label.obj" % (idx_beg + i)
            ),
        )
