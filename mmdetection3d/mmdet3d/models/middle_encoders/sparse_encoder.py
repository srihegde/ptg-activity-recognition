...
class SparseEncoderSASSD(SparseEncoder):
    ...
    def forward(self, voxel_features, coors, batch_size, test_mode=False):
        ...
        p0 = self.make_auxiliary_points(
            encode_features[0], points_mean, offset=(0, -40.0, -3.0), voxel_size=(0.1, 0.1, 0.2)
        )

        p1 = self.make_auxiliary_points(
            encode_features[1], points_mean, offset=(0, -40.0, -3.0), voxel_size=(0.2, 0.2, 0.4)
        )

        p2 = self.make_auxiliary_points(
            encode_features[2], points_mean, offset=(0, -40.0, -3.0), voxel_size=(0.4, 0.4, 0.8)
        )
    ...
    def make_auxiliary_points(
        self, source_tensor, target, offset=(0.0, -40.0, -3.0), voxel_size=(0.05, 0.05, 0.1)
    ):
