...
class GroupFree3DHead(BaseModule):
        ...
        for prefix in prefixes:
            ...
            if self.size_cls_agnostic:
                ...
                one_hot_size_targets_expand = (
                    one_hot_size_targets.unsqueeze(-1).expand(-1, -1, -1, 3).contiguous()
                )
                size_residual_norm = torch.sum(
                    bbox_preds[f"{prefix}size_res_norm"] * one_hot_size_targets_expand, 2
                )
                box_loss_weights_expand = box_loss_weights.unsqueeze(-1).expand(-1, -1, 3)
        ...
        if self.bbox_coder.with_rot:
            ...
            for i in range(gt_labels_3d.shape[0]):
                ...
                for j in range(self.gt_per_seed):
                    ...
                    if j == 0:
                        vote_targets_tmp[column_indices, : 3 * self.gt_per_seed] = votes[
                            column_indices
    ...
    def get_bboxes(self, points, bbox_preds, input_metas, rescale=False, use_nms=True):
        ...
        if self.test_cfg["prediction_stages"] == "last":
            ...
            prefixes = [
                f"s{i}." for i in range(self.num_decoder_layers - 3, self.num_decoder_layers)
            ]
        else:
            raise NotImplementedError
