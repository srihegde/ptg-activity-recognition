...
class PGDHead(FCOSMono3DHead):
    ...
    def _init_predictor(self):
        ...
        if self.use_depth_classifier:
            self.conv_depth_cls_prev = self._init_branch(
                conv_channels=self.depth_branch, conv_strides=(1,) * len(self.depth_branch)
            )
    ...
    def get_proj_bbox2d(
        ...
        for idx in range(num_imgs):
            ...
            if pos_strided_bbox_preds[mask].shape[0] == 0:
                continue
            cam2img = torch.eye(
                4, dtype=pos_strided_bbox_preds.dtype, device=pos_strided_bbox_preds.device
            )
            view_shape = views[idx].shape
            cam2img[: view_shape[0], : view_shape[1]] = pos_strided_bbox_preds.new_tensor(
                views[idx]
            )
    ...
    def get_pos_predictions(
        ...
        flatten_dir_cls_preds = [
            dir_cls_pred.permute(0, 2, 3, 1).reshape(-1, 2) for dir_cls_pred in dir_cls_preds
        ]
        flatten_centerness = [
        ...
        if self.weight_dim != -1:
            flatten_weights = [
                weight.permute(0, 2, 3, 1).reshape(-1, self.weight_dim) for weight in weights
            ]
    ...
    def loss(
        ...
        pos_inds = (
            ((flatten_labels_3d >= 0) & (flatten_labels_3d < bg_class_ind)).nonzero().reshape(-1)
        )
        ...
        if num_pos > 0:
            ...
            if self.use_depth_classifier:
                ...
                if self.weight_dim != -1:
                    loss_fuse_depth = self.loss_depth(
                        sig_alpha * pos_bbox_preds[:, 2] + (1 - sig_alpha) * pos_prob_depth_preds,
                        pos_bbox_targets_3d[:, 2],
                        sigma=pos_weights[:, 0],
                        weight=bbox_weights[:, 2],
                        avg_factor=equal_weights.sum(),
            ...
            if self.pred_keypoints:
                ...
                proj_bbox2d_preds, pos_decoded_bbox2d_preds, kpts_targets = self.get_proj_bbox2d(
    ...
    def _get_bboxes_single(
        ...
        mlvl_bboxes_for_nms = xywhr2xyxyr(
            input_meta["box_type_3d"](
                mlvl_bboxes, box_dim=self.bbox_coder.bbox_code_size, origin=(0.5, 0.5, 0.5)
            ).bev
        )
    ...
    def get_targets(
        ...
        bbox_targets_3d_list = [
            bbox_targets_3d.split(num_points, 0) for bbox_targets_3d in bbox_targets_3d_list
        ]
        centerness_targets_list = [
