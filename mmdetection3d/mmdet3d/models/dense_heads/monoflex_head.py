...
class MonoFlexHead(AnchorFreeMono3DHead):
    ...
    def get_bboxes(self, cls_scores, bbox_preds, input_metas):
        ...
        cam2imgs = torch.stack(
            [cls_scores[0].new_tensor(input_meta["cam2img"]) for input_meta in input_metas]
        )
        batch_bboxes, batch_scores, batch_topk_labels = self.decode_heatmap(
            cls_scores[0], bbox_preds[0], input_metas, cam2imgs=cam2imgs, topk=100, kernel=3
        )
    ...
    def get_predictions(
        self, pred_reg, labels3d, centers2d, reg_mask, batch_indices, input_metas, downsample_ratio
    ):
    ...
    def get_targets(
        ...
        keypoints2d_list, keypoints_mask_list, keypoints_depth_mask_list = get_keypoints(
            gt_bboxes_3d_list, centers2d_list, input_metas
        )
