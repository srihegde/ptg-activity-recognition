...
class SMOKEMono3DHead(AnchorFreeMono3DHead):
    ...
    def get_predictions(
        ...
        locations, dimensions, orientations = self.bbox_coder.decode(
            pred_regression_pois, centers2d, labels3d, cam2imgs, trans_mats, gt_locations
        )
    ...
    def get_targets(
        ...
        for batch_id in range(bs):
            ...
            for j, center in enumerate(gt_center2d):
                ...
                ind = gt_label[j]
                gen_gaussian_target(
                    center_heatmap_target[batch_id, ind], [center_x_int, center_y_int], radius
                )
    ...
    def loss(
        ...
        loss_bbox_oris = self.loss_bbox(
            pred_bboxes["ori"].corners[reg_inds, ...], target_labels["gt_cors"][reg_inds, ...]
        )

        loss_bbox_dims = self.loss_bbox(
            pred_bboxes["dim"].corners[reg_inds, ...], target_labels["gt_cors"][reg_inds, ...]
        )

        loss_bbox_locs = self.loss_bbox(
            pred_bboxes["loc"].corners[reg_inds, ...], target_labels["gt_cors"][reg_inds, ...]
        )
