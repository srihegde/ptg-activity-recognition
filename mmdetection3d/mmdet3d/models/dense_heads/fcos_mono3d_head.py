...
class FCOSMono3DHead(AnchorFreeMono3DHead):
    ...
    def forward_single(self, x, scale, stride):
        ...
        """
        cls_score, bbox_pred, dir_cls_pred, attr_pred, cls_feat, reg_feat = super().forward_single(
            x
        )
    ...
    @force_fp32(
        apply_to=("cls_scores", "bbox_preds", "dir_cls_preds", "attr_preds", "centernesses")
    )
    def loss(
        self,
        cls_scores,
        ...
        if num_pos > 0:
            ...
            if self.use_direction_classifier:
                pos_dir_cls_targets = self.get_direction_target(
                    pos_bbox_targets_3d, self.dir_offset, self.dir_limit_offset, one_hot=False
                )
    ...
    def _get_target_single(
        ...
        gt_bboxes_3d[..., 6] = (
            -torch.atan2(gt_bboxes_3d[..., 0], gt_bboxes_3d[..., 2]) + gt_bboxes_3d[..., 6]
        )
        ...
        relative_dists = torch.sqrt(torch.sum(bbox_targets_3d[..., :2] ** 2, dim=-1)) / (
            1.414 * stride[:, 0]
        )
        # [N, 1] / [N, 1]
