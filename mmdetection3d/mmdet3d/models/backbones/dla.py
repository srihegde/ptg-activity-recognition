...
class BasicBlock(BaseModule):
    ...
    def __init__(
        self, in_channels, out_channels, norm_cfg, conv_cfg, stride=1, dilation=1, init_cfg=None
    ):
...
class Tree(BaseModule):
    ...
    def __init__(
        ...
        if levels == 1:
            self.root = Root(
                root_dim, out_channels, norm_cfg, conv_cfg, root_kernel_size, add_identity
            )
            self.tree1 = block(
...
class DLANet(BaseModule):
    ...
    def _make_conv_level(
        self, in_channels, out_channels, num_convs, norm_cfg, conv_cfg, stride=1, dilation=1
    ):
        """Conv modules.
