...
class Custom3DSegDataset(Dataset):
    ...
    def get_classes_and_palette(self, classes=None, palette=None):
        ...
        if self.CLASSES:
            ...
            self.VALID_CLASS_IDS = [
                self.VALID_CLASS_IDS[self.CLASSES.index(cls_name)] for cls_name in class_names
            ]
    ...
    def evaluate(self, results, metric=None, logger=None, show=False, out_dir=None, pipeline=None):
        ...
        ret_dict = seg_eval(
            gt_sem_masks, pred_sem_masks, self.label2cat, self.ignore_index, logger=logger
        )
