...
class ObjectSample(object):
    ...
    def __call__(self, input_dict):
        ...
        if self.sample_2d:
            ...
            sampled_dict = self.db_sampler.sample_all(
                gt_bboxes_3d.tensor.numpy(), gt_labels_3d, img=None, ground_plane=ground_plane
            )
...
class PointSample(object):
    ...
    def _points_random_sampling(
        self, points, num_samples, sample_range=None, replace=False, return_choices=False
    ):
...
class IndoorPatchPointSample(object):
    ...
    def _patch_points_sampling(self, points, sem_mask):
        ...
        for _ in range(self.num_try):
            ...
            mask = (
                np.sum(
                    (cur_coords >= (cur_min - self.eps)) * (cur_coords <= (cur_max + self.eps)),
                    axis=1,
                )
                == 3
            )
        ...
        if isinstance(self.down_ratio, tuple):
            trans_mat = [
                self._get_transform_matrix(
                    center, size, (self.img_scale[0] // ratio, self.img_scale[1] // ratio)
                )
            ...
            trans_mat = self._get_transform_matrix(
                center,
                size,
                (self.img_scale[0] // self.down_ratio, self.img_scale[1] // self.down_ratio),
            )
