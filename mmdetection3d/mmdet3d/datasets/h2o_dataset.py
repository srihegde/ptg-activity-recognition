...
class H2ODataset(Custom3DDataset):
    CLASSES = ('dontcare', 'book', 'espresso', 'lotion', 'spray', 'milk', 
               'cocoa', 'chips', 'capuccino ', 'cabinet', 'bed', 'chair',
    ...
    def load_annotations(self, ann_file):
        ...
        data_infos = [ dict(idx=i,
                            vid_path='/'.join(fp.split('/')[:-2]),
                            frm_name=fp.split('/')[-1].split('.')[0],
                            img_prefix='/'.join(fp.split('/')[:-1])) for i,fp in enumerate(data[:10])]
        for di in data_infos:
            di.update(dict(pc_path=osp.join(di['vid_path'],'pc',f'{di["frm_name"]}.txt'),
    ...
    def get_data_info(self, index):
        ...
        img_info = dict(filename=f"{info['frm_name']}.png",
    ...
    def get_ann_info(self, index, cam_matrix):
        ...
        """
        info = self.data_infos[index]
        gt_labels_3d, gt_bboxes_3d, gt_obj_centers, depth = self._load_obj_data(info['obj_meta'], cam_matrix)
        gt_bboxes_3d = DepthInstance3DBoxes(
            gt_bboxes_3d,
            box_dim=gt_bboxes_3d.shape[-1]).convert_to(self.box_mode_3d)
    ...
    def _load_obj_data(self, annotation_file, camera_matrix):
        with open(annotation_file) as f:
            ...
            for lines in all_lines:
                ...
                all_obj_pose.append(obj_pose.unsqueeze(0).long())
            all_obj_pose = torch.cat(all_obj_pose)
            
            obj_centers, _ = cv2.projectPoints(np.array(obj_centers), np.array([[0, 0, 0]]).astype(
    ...
    def evaluate(self,
        ...
        for info in self.data_infos:
            ...
            info['annos'] = dict(gt_num=obj_pose.shape[0],
                                 gt_boxes_upright_depth=obj_pose)
