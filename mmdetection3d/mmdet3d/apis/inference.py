...
def init_model(config, checkpoint=None, device="cuda:0"):
    ...
    if device != "cpu":
        ...
        logger.warning(
            "Don't suggest using CPU device. " "Some functions are not supported for now."
        )
    model.to(device)
    model.eval()
    return model
...
def show_seg_result_meshlab(data, result, out_dir, palette, show=False, snapshot=False):
    ...
    show_seg_result(
        points, None, pred_seg, out_dir, file_name, palette=palette, show=show, snapshot=snapshot
    )
...
def show_result_meshlab(
    data, result, out_dir, score_thr=0.0, show=False, snapshot=False, task="det", palette=None
):
