...
class MonoFlexCoder(BaseBBoxCoder):
    ...
    def decode(self, bbox, base_centers2d, labels, downsample_ratio, cam2imgs):
        ...
        pred_direct_depth_uncertainty = torch.clamp(
            pred_direct_depth_uncertainty, self.uncertainty_range[0], self.uncertainty_range[1]
        )
        pred_keypoints_depth_uncertainty = torch.clamp(
            pred_keypoints_depth_uncertainty, self.uncertainty_range[0], self.uncertainty_range[1]
        )

        if self.combine_depth:
            pred_depth_uncertainty = torch.cat(
                (pred_direct_depth_uncertainty.unsqueeze(-1), pred_keypoints_depth_uncertainty),
                dim=1,
            ).exp()
    ...
    def decode_location(
        self, base_centers2d, offsets2d, depths, cam2imgs, downsample_ratio, pad_mode="default"
    ):
    ...
    def decode_orientation(self, ori_vector, locations):
        ...
        if self.multibin:
            ...
            for i in range(self.num_dir_bins):
                ...
                orientations[mask_i] = (
                    pred_bin_offset[:, 0].atan2(pred_bin_offset[:, 1]) + self.bin_centers[i]
                )
        else:
