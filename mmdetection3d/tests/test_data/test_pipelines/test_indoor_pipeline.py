...
def test_scannet_seg_pipeline():
    ...
    scannet_points = np.fromfile(osp.join(data_path, info["pts_path"]), dtype=np.float32).reshape(
        (-1, 6)
    )
...
def test_s3dis_seg_pipeline():
    ...
    # build sampled points
    s3dis_points = np.fromfile(osp.join(data_path, info["pts_path"]), dtype=np.float32).reshape(
        (-1, 6)
    )
