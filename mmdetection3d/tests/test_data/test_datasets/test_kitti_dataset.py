...
def test_getitem():
    ...
    expected_rot_matrix = torch.tensor(
        [[0.8018, 0.5976, 0.0000], [-0.5976, 0.8018, 0.0000], [0.0000, 0.0000, 1.0000]]
    )
    ...
    expected_gt_bboxes_3d[:, -1:] = (
        -np.pi - expected_gt_bboxes_3d[:, -1:] + 2 * rot_angle - 2 * noise_angle
    )
    ...
    modality = dict(use_lidar=True, use_camera=True)
    kitti_dataset = KittiDataset(
        data_root, ann_file, split, pts_prefix, multi_modality_pipeline, classes, modality
    )
    data = kitti_dataset[0]
    img = data["img"]._data
...
def test_format_results():
    ...
    assert np.allclose(result_files[0]["rotation_y"], expected_rotation_y, 1e-3)
