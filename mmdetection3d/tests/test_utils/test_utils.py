...
def test_points_img2cam():
    ...
    expected_xyzs = torch.tensor([[-0.4864, -0.2155, 0.7576], [-0.6299, -0.2796, 0.9813]])
...
def test_truncation_hanlde():
    ...
    centers2d_list = [torch.tensor([[-99.86, 199.45], [499.50, 399.20], [201.20, 99.86]])]

    gt_bboxes_list = [
        torch.tensor(
            [[0.25, 99.8, 99.8, 199.6], [300.2, 250.1, 399.8, 299.6], [100.2, 20.1, 300.8, 180.7]]
        )
    ]
    ...
    centers2d_target = torch.tensor([[0.0, 166.30435501], [379.03437877, 299.0], [201.2, 99.86]])
