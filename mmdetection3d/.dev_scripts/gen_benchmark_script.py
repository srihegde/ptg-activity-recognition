...
def parse_args():
    parser = argparse.ArgumentParser(description="Generate benchmark training/testing scripts")
    parser.add_argument(
        "--input_file",
        required=False,
        type=str,
    ...
    parser.add_argument("--full_log", action="store_true", help="Whether save full log in a file")
...
if args.input_file is not None:
    ...
    while True:
        print("Please type a config path and " "press enter (press enter directly to exit):")
        config_path = input()
        if config_path != "":
...
if args.mode == "train":
    for i, config_path in enumerate(config_paths):
        ...
        if match_obj is None:
            print(f"Invalid config path (no GPU num in " f"config name):\n{config_path}")
            continue
    ...
    for i, config_path in enumerate(config_paths):
        ...
        tasks = {"scannet_seg", "scannet", "s3dis_seg", "sunrgbd", "kitti", "nus", "lyft", "waymo"}
        eval_option = None
        for task in tasks:
