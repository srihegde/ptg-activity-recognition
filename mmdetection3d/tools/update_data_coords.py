...
def update_sunrgbd_infos(root_dir, out_dir, pkl_files):
    print(
        f"{pkl_files} will be modified because " f"of the refactor of the Depth coordinate system."
    )
    if root_dir == out_dir:
...
def update_outdoor_dbinfos(root_dir, out_dir, pkl_files):
    print(
        f"{pkl_files} will be modified because " f"of the refactor of the LIDAR coordinate system."
    )
    if root_dir == out_dir:
        print(f"Warning, you are overwriting " f"the original data under {root_dir}.")
        time.sleep(3)
...
parser.add_argument(
    "--root-dir", type=str, default="./data/kitti", help="specify the root dir of dataset"
)
parser.add_argument(
    "--version",
    type=str,
